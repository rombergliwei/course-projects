...
for k = 1:N
    t = -3:1/fs:+3;%给t数值
    a = 2*(1-(-1)^k)/((k^2)*((T2-T1)/2));%自己求出傅里叶级数的系数给出表达式
    y = y + a*cos(k*t);
    ...
    if k==1
        imwrite(I,map,'10_1_3的傅里叶级数重构.gif','GIF', 'Loopcount',inf,'DelayTime',0.1);
    else
        imwrite(I,map,'10_1_3的傅里叶级数重构.gif','GIF','WriteMode','append','DelayTime',0.1);
    end
end