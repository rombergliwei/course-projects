...
#include <iomanip>
#include <cmath>
using namespace std;//我们一下使用的求解方程的方法使用的是课件中的迭代的形式，并不是课件中的算法 
const double epsilon=0.00000001;//设定的最小误差 
const int N=9;//我们要计算的是九阶方阵 
int Gauss_Seidel(double array[][N],double array1[],double x[])//下面是GS迭代法求解的函数 
{
	...
	for (i=0;i<N;i++)
		for (j=0;j<N;j++)
			matrix[i][j]=array[i][j];//先一个一个将传过来的矩阵赋值给这里的新的矩阵 
	for (i=0;i<N;i++) y[i]=array1[i];//将传过来的列向量赋值给这里的新的向量 
...
int SOR(double array[][N],double array1[],double x[],double w)//下面开始使用SOR方法求解 
{
	double t,max_error=100;
	double matrix[N][N];
	double y[N];
...
int main()
{
	double array[N][N]={//这里是要计算的矩阵，可以进行修改 
    ...
    double array1[N]={-15,27,-23,0,-20,12,-7,7,10};//这里是矩阵的右边的值 
   ...
   for( i=1;i<100;i++)
    {
    	counter=SOR(array,array1,x,i/50.0);//不停改变松弛因子然后进行计算SOR的计算步数 
